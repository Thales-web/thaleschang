...
import type {
  SidebarSection,
  SidebarTab,
} from "@/docs/config/types/docsConfigTypes";
...
export const docsRoute = (docsSiteSettings.docsRoute || "docs").replace(
  /^\/|\/$/g,
  "",
);

// Cache for translated tab data to avoid repeated data fetching
const tabCache = new Map<LocaleType, SidebarTab[]>();

/**
 * Get translated tabs data with caching
 */
const getTranslatedTabs = (locale: LocaleType): SidebarTab[] => {
  if (!tabCache.has(locale)) {
    tabCache.set(
      locale,
      getTranslatedData("sidebarNavData", locale).tabs,
    );
  }
...
export const getSectionById = (
  ...
  return getTabSections(tabId, locale).find(
...
/**
 * Get all non-draft docs filtered by locale
 */
export const getAllDocs = async (locale?: LocaleType) => {
  const allDocs = await getCollection("docs", ({ data }) => {
    return data.draft !== true;
  });

  if (locale) {
    return filterCollectionByLanguage(allDocs, locale);
  }
